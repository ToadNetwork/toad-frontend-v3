        ...
        <v-card
          ...
          <div class="text-center">
            ...
            <h5 class="stats-line">
              <img :src="toad_img" style="height:25px">
              Daily Return
              <span class="stats-line-info">${{round(Number(stake_value)*Number(roi)/100,4)}}</span>
            </h5>
            <v-divider style="margin-left: 2%; width: 96%;"></v-divider>
            <h5 class="stats-line">
              Toad Price
...
import { lpFarm, tokenABI, toadAddress } from '../farms_config.json'
export default {
  components: {
    flipcard: FlipCard
  },
  ...
  methods: {
    ...
    async loadInstances () {
          ...
          this.tokenAddress = lpFarm.tokenAddress
          this.contractAddress = lpFarm.contractAddress
          this.contractABI = lpFarm.contractABI
    ...
    async getPoolInfo(){
      ...
      this.pool_size = this.round(Number(amount)/1e18, 2) + ' TOAD-BNB'
    ...
    async getROI(){
      const instance = new this.dataseed.eth.Contract(tokenABI, '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c')
    ...
    async deposit() {
      ...
      await contractInstance.methods.buy(amount).send(args)
    },
    async remove(){
      const contractInstance = new this.web3.eth.Contract(this.contractABI, this.contractAddress)
      let amount = this.web3.utils.toWei(this.amount.replace(',','.'), 'ether')
